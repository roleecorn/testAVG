...
- Before declaring any story creation or adjustment complete, perform the full-width-bracket directive audit defined in [text-to-event-json.md](references/text-to-event-json.md): inspect every `【...】` occurrence in the authoritative source, treat it as an演出／AI instruction unless the source contract explicitly says otherwise, and first verify whether the current generator and runtime event schema can implement it. If it can be implemented by extending the generator or its required shared runtime mapping, do that before creating a TODO; only unresolved assets, ambiguous or unfinished source intent, or a capability still impossible after the feasibility check may remain open. Prove the final intent in validated Story IR and the corresponding floor, and stop the affected branch while any unresolved directive remains.
- Source boundary: `project/mainStory/` and `project/story/*.txt` are authoritative inputs. The Agent must never author, partially edit, polish, correct, reformat, delete, move, rename, or reverse-generate their content. It may create a new source file or replace an existing source file in full only from a confirmed, traceable user/ZIP/external source whose text is preserved without Agent rewriting; such source files may be staged and committed only with their matching Story IR and scene/floor transaction. Treat `project/story/manifest.md` and `project/story/TODO.md` as mutable traceability metadata, never as story authority.
...
- Start every ZIP execution from the user-designated original archive. Record its SHA-256, create a new verified-empty run directory under `tmp/character-story-import/<archive-base>/<run-id>/`, freshly extract the archive into `raw/`, and generate this run's manifests, extracted text, draft IR, and other intermediates only under `work/`. Never reuse, copy forward, or trust a prior `tmp` directory or prior extraction, manifest, text extraction, image inventory, character resolution, or draft IR; rerun extraction and inventory even when the archive path and filename are unchanged. Treat similarly named artifacts contained inside the ZIP as untrusted payload, not current-run evidence.
- Before landing source or integrating assets, classify every freshly extracted script against the current authoritative `project/story/` source as `new`, `updated`, `identical`, or `conflict`, and classify every freshly extracted image against current repository/runtime assets as `new`, `identical-existing`, `changed-existing`, or `unresolved`. Record complete-content hashes and diffs for text; record actual format, dimensions, SHA-256, and visual comparison evidence for images. Never infer addition or update from filenames, extensions, or counts. Stop the affected branch on `conflict` or `unresolved`.
...
- Merge every finalized per-character script and asset record from the current run into `project/story/manifest.md`: preserve archive/run identity, original relative path and SHA-256, resource kind, delta status, usage method, final filename/path, target scene or story source, and validation evidence. Preserve prior rows as `superseded` when resources are renamed, replaced, or retired. Never invent historical provenance; mark unverifiable legacy sections `needs-backfill`.
