...
- Before declaring any story creation or adjustment complete, perform the full-width-bracket directive audit defined in [text-to-event-json.md](references/text-to-event-json.md): inspect every `【...】` occurrence in the authoritative source, interpret it in full story context, and record the semantic intent in the Agent-authored Story IR. If the intent is unresolved or runtime cannot express it, stop the affected branch and record the original directive, scope, and blocker in question/TODO; do not teach a generator a one-off hardcoded interpretation as a substitute for semantic IR.
