...
- Before declaring any story creation or adjustment complete, perform the full-width-bracket directive audit defined in [text-to-event-json.md](references/text-to-event-json.md): inspect every `【...】` occurrence in the authoritative source, prove its intended演出 is represented by the validated Story IR and corresponding floor, and record any conflict or unresolved directive in the applicable story TODO. Stop the affected branch and do not claim completion while a conflict remains.
...
- Run the complete `【...】` directive audit for every story creation or adjustment; verify each directive against Story IR and floor evidence, and route every conflict or unresolved directive to the applicable TODO before delivery.
