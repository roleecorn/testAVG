...
- 現行畫布：544×416（17×13）。
- 場景初始背景：必須是 544×416，並預載於樓層 `images` 的 `canvas: "bg"`、`x: 0, y: 0`，讓它在 map 與 `eachArrive` 之前完成繪製。`eachArrive` 中的 `showImage` 可維持動態背景生命週期與後續切換，但不能作為唯一的進場背景，否則第一幀可能露出預設石磚。416×416 背景或 placeholder 均為錯誤，必須替換，不得縮放、裁切或保留作為相容方案。
...
因此 AVG 驗收必須檢查每個 scene 都有 floor-level 初始背景；看到 `eachArrive` 第一個背景事件並不足以證明進場無閃屏。
