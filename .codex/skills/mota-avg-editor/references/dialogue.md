...
目前標準遊戲畫布是 544×416（17×13）。新版普通 AVG 的構圖固定為「單一當前發言者－下方對話框」：畫面只保留一個普通人物位置，人物可見 bbox 左右置中於畫面，且可見 bottom 精準錨定於畫面外 `portraitBottomY: 440`，下半身延伸到對話框後並由 viewport 底部裁切。對話框不再為左右人物槽刻意縮窄；新版全局 layout config 的基準矩形為 `x=16, y=295, width=512, fixedLines=2`，人物圖層位於對話框 UI 後方。不得回用舊的 `x=96, width=352` 窄框值，也不得把幾何值散落在 floor 或逐句事件。

人物不再使用左右槽定位。人物可見 bbox 的水平中心必須等於畫面水平中心，可見底邊必須等於 `portraitBottomY`；角色 mapping、floor、scene 與逐句事件都不得增加專屬偏移。

人物圖層必須低於對話框 UI，且可見內容應停在對話框正文區上方。人物不得使用會越過 UI、遮住角色名或正文的圖片層級。

普通單人立繪台詞固定使用 `showImage(本句人物 code) → dialogue → hideImage(同一 code)`。hide 發生在該句對話結束後，不是在下一句開始前；不得清除本句未顯示的 code，也不得用跨句 active state 推測目前存在的圖片。分歧中的每條 action 都各自遵守此原子順序，因此不會共享人物狀態。來源明確要求多人同時存在時，才建立對應的明確多圖事件。
