...
目前標準遊戲畫布是 544×416（17×13）。新版普通 AVG 的構圖固定為「單一當前發言者－下方對話框」：畫面只保留一個普通人物位置，人物可見 bbox 左右置中於畫面，且可見 top 精準錨定於畫面高度的 20%（`portraitTopRatio: 0.2`）。對話框不再為左右人物槽刻意縮窄；新版全局 layout config 的基準矩形為 `x=16, y=295, width=512, fixedLines=2`，人物圖層位於對話框 UI 後方。不得回用舊的 `x=96, width=352` 窄框值，也不得把幾何值散落在 floor 或逐句事件。

人物不再使用左右槽定位。人物可見 bbox 的水平中心必須等於畫面水平中心，可見頂端必須等於 `viewportHeight * portraitTopRatio`；角色 mapping、floor、scene 與逐句事件都不得增加專屬偏移。
