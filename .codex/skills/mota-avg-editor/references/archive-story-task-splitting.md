...
**版本控制：** 每位角色完成 `G` 的整合驗收後，必須立刻為該角色的完整變更建立一次獨立 Git commit。該 commit 必須同時包含本次新增或整檔覆蓋的 `project/story/` 真實故事 TXT、`project/story/manifest.md` 中只屬於本角色的紀錄、受影響 `project/story-ir/`、所有對應 scene／floor、被 scene 實際引用的 runtime 素材、事件入口、共用註冊檔中只屬於該角色的行、該角色根層 `unknown/` 待辦圖片，以及必要 TODO／流程變更。`unknown/` 圖片不得加入 `main.images`；只作為生成來源且未被 runtime 直接使用的原始 ZIP 圖也不得放入 `project/images/`。Story IR 不得獨自提交，也不得把 scene／floor 延後到另一個 commit。不可把多位角色混在同一個 commit；若 ZIP 含多位角色，必須依角色重複 A 至 G，並在每位角色完成後各自 commit。提交前先確認 `git diff --cached --name-only`、`git diff --cached`、`asset-usage.md` 與 `project/story/manifest.md` 同時證明來源、原始資源使用方法與最後命名、IR、對應 scene／floor、`project/images → main.images → scene` 鏈及 `unknown/` TODO；共用檔案不得整檔加入。提交後立即用 `git show --stat --name-only <commit>` 複核，確認沒有跨角色、source-only、IR-only、缺漏 manifest、未分類來源圖、未登錄 images 圖、無 scene 使用的 data 登錄或無關檔案。所有角色 commit 完成後，只有在劇情更新完成（本次更新邊界內所有 hash drift 均已解決）、完整 `node scripts/validate_story.js` 成功且全域沒有來源 SHA-256 mismatch、stale IR、hash drift 或其他同步阻塞時，才可建立最後一個只更新 `AGENTS.md` 本節基準雜湊的 commit，將最後一個角色內容 commit 的完整雜湊寫入；該基準 commit 不得混入任何劇情、圖片、註冊或 TODO 變更。任何既有 drift 都不得因不屬於本角色而略過，必須維持原 baseline，也不得把未完成的角色變更建立成完成內容 commit。
