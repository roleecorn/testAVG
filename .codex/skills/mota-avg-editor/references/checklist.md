...
- 主線來源更新若造成 CG 首次出現的實體行號變化，已在重新產生 IR／floor 前逐一改名受影響的 `CH<N>_L<N>.png` CG，並核對每個檔名指向完整主線中的實際首次 CG 指令、每個後續相同 CG（包含跨章節）都重用該檔案；無法證明舊新對應的 CG 已停止受影響分支並建立 question／TODO。
