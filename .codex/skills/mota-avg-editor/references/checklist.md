...
- 主線生成器已先執行 `node scripts/generate_main_story.js --check`；所有標準主線 floor 均為 17×13，且生成器沒有把地圖寫回 13×13。
- `[人名：內容]` 簡訊仍輸出為帶角色名的手機對話；帶冒號的長敘事方括號仍是旁白，生成器沒有把兩者互相誤判，也沒有自行改寫來源台詞用字。
- 六張已登錄的主線動作 CG 都能由目前來源中的 CG／GIF 名稱命中；來源標記改名時不會靜默退回 placeholder。
