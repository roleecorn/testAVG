...
右下角 `(16,12)` 的 `idle_clock` 是人物交流回合的保底互動點，圖示由 `Akiba.js -> images` 的 `akiba_idle_clock.png` 顯示。點擊時梗平會說「啥也不想做啊」；只有 `mainline_exchange_active` 為真時，才會讓 `mainline_exchange_count` 加一。未達標時把角色移回本次時鐘互動前的座標，達標時才前往主線的續接 scene。
...
角色支線文本若提供 `場地:`，新增支線事件時應先查 `project/location-mappings.json` 的 `floors.Akiba.locations`，把文本地點對應到既有 `location.id`：

1. 優先使用店名完全相同或明顯同義的地點。
2. 若沒有完全命中，自動尋找語意最接近的既有地點，不要因為地點名稱不存在就拒絕新增支線。例如「雜貨店」可對應便利店或便利商店類地點。
3. 若找不到合理近似地點，選用一個既有地點作為隨機 fallback，並在交付說明中寫明 fallback 的 `location.id`、顯示名稱，以及「因文本地點不存在且無近似地點」。
4. 除非使用者明確要求新增地圖地點，不要為了支線文本中的新地點修改秋葉原地圖或 `location-mappings.json`。
