...
- 若有 TODO、待補內容、素材缺口、未定演出或待確認人物，已新增或更新 TODO list 檔案；不可只在對話框內列出。
- 不確定人物已用 `不知道是誰的<劇本中出現的名稱>` 暫代，並記錄到 TODO list 的待確認人物；未經使用者確認前，不自行替換正式角色 ID 或套用圖片。
- 缺少 CG/GIF/BGM 時，已複製同類型現有素材並命名成劇情需要的新檔名，已登錄並引用新檔名；TODO list 的 `待補素材` 已標明暫用來源與之後需替換的正式素材。
- `【CG：...】`、`【GIF ...】` 已轉成實際素材顯示；事件中不可殘留玩家可見的 `【CG：...】`、`【GIF：...】`、`\t[CG]...`、`\t[GIF]...` 文字描述。
- `【背景：...】` 已轉成樓層背景或 `showImage`；事件中不可把背景製作指令當成玩家可見文字。
