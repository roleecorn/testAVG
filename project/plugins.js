...
var plugins_bb40132b_638b_4a9f_b028_d3fe47acc8d1 =
{
	...
	"Tic_Tac_Toe": function () {
		...
		function loadMiniGame(gameId, callback) {
			if (getMiniGame(gameId)) return callback();
			if (loadingCallbacks[gameId]) {
				loadingCallbacks[gameId].push(callback);
				return;
			}
			loadingCallbacks[gameId] = [callback];

			var script = document.createElement("script");
			script.src = "extensions/minigames/" + gameId + ".js";
			script.onload = function () {
				var callbacks = loadingCallbacks[gameId] || [];
				delete loadingCallbacks[gameId];
				callbacks.forEach(function (one) { one(); });
			}
			script.onerror = function () {
				var callbacks = loadingCallbacks[gameId] || [];
				delete loadingCallbacks[gameId];
				callbacks.forEach(function (one) { one(new Error("loadMiniGameFailed:" + gameId)); });
			}
		...
		this.startMiniGame = function (gameId, options, callback) {
			...
			loadMiniGame(gameId, function (error) {
				var game = getMiniGame(gameId);
				if (error || !game || !game.start) {
					if (callback) callback({ result: "error", reason: "loadFailed" });
					return;
				}
				currentGame = game.start(options, function (result) {
					currentGame = null;
		...
		this.startTicTacToeDemoEvent = function () {
			this.startMiniGame("ticTacToe", {}, function (result) {
				var reward = 0;
				var text = "\t[小遊戲]圈圈叉叉已結束。";
				core.setFlag("lastMiniGameResult", result.result);
				core.setFlag("lastMiniGameScore", result.score || 0);

				if (result.winner) core.setFlag("lastMiniGameWinner", result.winner);
				else core.removeFlag("lastMiniGameWinner");

				if (result.result === "win") {
					reward = 100;
					text = "\t[小遊戲]結果：" + result.winner + " 獲勝！獎勵金錢 +" + reward + "。";
				} else if (result.result === "draw") {
					reward = 20;
					text = "\t[小遊戲]結果：平手。獎勵金錢 +" + reward + "。";
				} else if (result.result === "cancel") {
					text = "\t[小遊戲]你中止了圈圈叉叉，沒有獲得獎勵。";
				}
